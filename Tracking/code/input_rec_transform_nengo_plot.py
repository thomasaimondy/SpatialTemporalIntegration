# %load input_rec_transform_nengo_plot.py
...
def plot_data(dataFileName,endTag):
    ...
    with contextlib.closing(
        ...
        if N>4 and N != 7: [ax2.plot(trange, y2[:,i], color=cnames[i], linewidth=1, label='L2') for i in range(N)]
        elif N == 7: [ax2.plot(trange, y2[:,i], color=cnames[i], linewidth=1, label='pred $\hat{x}$') for i in range(N)]
...
#     plot_rec_nengo_all(sys.argv[1])
    plot_rec_nengo_all("data/ff_rec_Nexc5000_norefinptau_directu_seeds2345_weightErrorCutoff0.0_nodeerr_learn_rec_nocopycat_func_rr__seed2by0.3amplVaryHeights_10.0s")
